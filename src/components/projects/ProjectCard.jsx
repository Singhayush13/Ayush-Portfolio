...
const ProjectCard = ({ images, title, description, githubLink, demoLink, techStack = [] }) => {
  ...
  useEffect(() => {
    ...
    const interval = setInterval(() => {
      ...
      const nextIndex = (currentImageRef.current + 1) % images.length;
      ...
      gsap.to(imgRef.current, {
        ...
        ease: "power2.inOut",
        onComplete: () => {
          currentImageRef.current = nextIndex;
  ...
  const onMouseEnter = () => {
    ...
    gsap.to(imgRef.current, { scale: 1.06, duration: 1, ease: "power2.out" });
    gsap.to(cardRef.current?.querySelector(".overlay-gradient"), { opacity: 0.4, duration: 0.6 });
  };
  ...
  const onMouseLeave = () => {
    ...
    gsap.to(cardRef.current?.querySelector(".overlay-gradient"), { opacity: 0.7, duration: 0.6 });
  };
  ...
  return (
    <div
      ...
      className={`group relative w-full h-[500px] md:h-[400px] rounded-[1.5rem] overflow-hidden border flex flex-col md:flex-row transition-all duration-500 ${isDark
    ...
    >
      ...
      >
        ...
        <div className="flex items-center gap-5 pt-6">
          {demoLink ? (
            ...
            <span className="text-[11px] font-black uppercase tracking-widest opacity-30">Coming soon</span>
          )}
          {githubLink && (
            <a href={githubLink} target="_blank" rel="noreferrer" aria-label={`${title} GitHub repository`} className={`opacity-40 hover:opacity-100 transition-opacity ${isDark ? 'text-white' : 'text-black'}`}>
              <FaGithub size={20} />
            </a>
          )}
        </div>
      ...
      <div className="relative flex-1 h-full overflow-hidden bg-zinc-900">
        ...
        <div className={`overlay-gradient absolute inset-0 transition-opacity duration-700 pointer-events-none hidden md:block ${isDark
